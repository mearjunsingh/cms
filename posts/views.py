...
class PostDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["related_posts"] = (
            ...
            .filter(author=context["post"].author)
            .exclude(id=context["post"].id)
            .order_by("-modified_date")[:3]
        )
        ...
        context["comments"] = (
            Comment.objects.filter(post=context["post"])
...
class AuthorList(BaseListView):
    def get_queryset(self):
        posts = (
            Post.objects.filter(is_published=True)
            .filter(author__username=self.kwargs.get("auth"))
            .order_by("-modified_date")
        )
        term = self.request.GET.get("search")
        if term:
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["list_term"] = User.objects.get(
            username=self.kwargs.get("auth")
        ).display_name
